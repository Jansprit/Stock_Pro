      ...
      <div className="grid w-full max-w-3xl grid-cols-2 gap-2 sm:grid-cols-4">
        {POPULAR.map((p) => (
          <button
            ...
            className="
              group flex flex-col items-start rounded-lg border border-slate-800
              bg-slate-900/60 px-4 py-3 text-left
              transition-all hover:border-brand-500/50 hover:bg-slate-800/60
            "
          >
            <span className="font-mono text-sm font-semibold text-slate-100 group-hover:text-brand-400">
              {p.symbol}
            </span>
            <span className="mt-0.5 text-xs text-slate-500">{p.name}</span>
          </button>
        ))}
      </div>
